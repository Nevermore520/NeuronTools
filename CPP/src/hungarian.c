        ...
        for (i = 0; i < n; i++)
	{
	    ...
	    *(table + i) = malloc(m * sizeof(cell));
	    for (j = 0; j < m; j++)
			...
			*(*(table + i) + j) = *(*(t + i) + j) = (cell)((4-i)*(2*i+j+1)*0.124);
	}
    else
    {
        cell x;
...
void print(cell** t, size_t n, size_t m, ssize_t** assignment)
{
	...
	for (j = 0; j < m; j++)
	{
	    ...
	    printf("%f%s\033[m   ", (cell)(*(*(t + i) + j)), (*(*(assigned + i) + j) ? "^" : " "));
        }
	printf("\n\n");
...
{
    size_t j = i >> 6L;
	llong old = *(thisBS.limbs + j);
    
	*(thisBS.limbs + j) |= 1LL << (llong)(i & 63L);
    
	if ((!*(thisBS.limbs + j)) ^ (!old))
    {
        j++;
		*(thisBS.prev + *(thisBS.first)) = j;
		*(thisBS.prev + j) = 0;
		*(thisBS.next + j) = *(thisBS.first);
		*(thisBS.first) = j;
    }
}
...
void BitSet_unset(BitSet thisBS, size_t i)
{
	...
	*(thisBS.limbs + j) &= ~(1LL << (llong)(i & 63L));
    ...
    {
        j++;
		size_t p = *(thisBS.prev + j);
		size_t n = *(thisBS.next + j);
		*(thisBS.prev + n) = p;
		*(thisBS.next + p) = n;
		if (*(thisBS.first) == j)
			*(thisBS.first) = n;
 ...
 */
ssize_t BitSet_any(BitSet thisBS)
{
	size_t i = *(thisBS.first) - 1;

	if (*(thisBS.first) == 0L)
        return -1;
    
	return (ssize_t)(lb(*(thisBS.limbs + i) & -*(thisBS.limbs + i)) + (i << 6L));
}
